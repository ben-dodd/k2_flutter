...
Future<ParsedResponse<List<JobHeader>>> getAllWfmJobs() async {
  ...
  http.Response response = await http
      .get(Strings.apiRoot + 'wfm/job.php?apiKey=' + Strings.apiKey)
      .catchError((resp) {});
...
Future logTime(xml.XmlDocument TimeSheet, xml.XmlDocument Assign) async {
  http
      .put(
          Strings.wfmRoot +
              'job.api/assign?apiKey=' +
              Strings.wfmApi +
              '&accountKey=' +
              Strings.wfmAccount,
          body: Assign.toString())
      .then((ass) {
    if (ass != null) {
...
//                          return new ParsedResponse(response.statusCode, json.decode(response.body));
      ...
      print('null reponse from assign');
  });

  print(Strings.wfmRoot +
      'time.api/add?apiKey=' +
      Strings.wfmApi +
      '&accountKey=' +
      Strings.wfmAccount);
