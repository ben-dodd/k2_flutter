...
class AcmRoomCard extends StatefulWidget {
  AcmRoomCard({
    @required this.doc,
...
//  final VoidCallback onCardLongPress;
  ...
  @override
  _AcmRoomCardState createState() => new _AcmRoomCardState();
}

class _AcmRoomCardState extends State<AcmRoomCard> {
  String name;

  bool hasPhoto;
  bool photoSynced;

  Widget _AcmRoomCard(Map<String, dynamic> doc) {
...
//              return _AcmRoomCard(new Map<String,dynamic>.from(doc['children'][index]));
//            },
...
//          )]
          doc['children'].map<Widget>((child) {
              // Start streams here?
              return _AcmRoomCard(new Map<String, dynamic>.from(child));
            }).toList()
          : [new Container()],
    );
  }
  ...
  Widget build(BuildContext context) {
    return _AcmRoomCard(widget.doc);
  }
}
